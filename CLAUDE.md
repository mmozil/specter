...
- **Fonte/cor = Harvey.ai real (inspecionado via browser + comparação visual)**: fundo `#0f0e0d` (= `--ink`, idêntico ao Harvey) + texto ivory/branco. Harvey usa fontes **proprietárias** (`HarveySerifFont` custom + `HarveySansDiatypeVariable` = **Diatype/Dinamo**, pagas, fora do Google Fonts → NÃO embarcáveis). Serif final = **Source Serif 4** (`--font-serif`, variável — escolha do dono: sóbria/legível, serifas crisp; contraste menor que a Harvey, mas preferida) + **Inter** (`--font-sans`, ≈ Diatype). *(Histórico de testes contra a HarveySerif real: Newsreader=pontas arredondadas ❌, Playfair=contraste alto demais, Libre Caslon/Prata=leves demais (single-weight 400), Fraunces=mais fiel à Harvey mas o dono optou por Source Serif 4.)* Harvey é **monocromática, sem accent cromático**: o dourado `#c9a96e` (resíduo Murdock) foi REMOVIDO — `--accent` = `#fafaf9` (ivory); botões primários = ivory bg + ink text (igual "Request a Demo"). `--green`/`--red` só como feedback semântico.
