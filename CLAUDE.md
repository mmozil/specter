...
- **Login com Google** (Google Identity Services): `GET /auth/config` expõe `google_client_id`; `POST /auth/google` verifica o ID token no `oauth2.googleapis.com/tokeninfo` (valida `aud`/`iss`/`email_verified`) e faz upsert do usuário. Frontend renderiza o botão GIS (tema dark) **só se** `GOOGLE_CLIENT_ID` estiver setado (graceful). **Pendência operacional**: criar OAuth Client ID (Web) no Google Cloud Console com origem `https://specter.hovio.com.br`, setar `GOOGLE_CLIENT_ID` no Coolify.
- **Admin** (`/llm-providers*`, `/feeds*`) seguem em `X-API-Key` (separado do login de usuário).
- **Design = Harvey.ai real (inspecionado via browser + comparação visual)**. Dois temas, igual à Harvey:
  - **Login/marketing = DARK** (`--ink #0f0e0d` + ivory) — overlay full-screen.
  - **App logado = CLARO/branco** (estilo in-app `app.harvey.ai`): sidebar off-white `#fafaf9`, área branca, título serif central, prompt com **botão de envio escuro** (igual "Ask Harvey"), cards de exemplo brancos com borda sutil. Implementado como bloco de override de tema claro no fim do `<style>` (login + modais admin LLM/feeds seguem dark). Commit `ad2ae43`.
  - **Monocromática, sem accent cromático**: dourado `#c9a96e` (resíduo Murdock) REMOVIDO → `--accent` = ivory; botões primários ivory+ink (dark) / dark+ivory (light).
  - **Fontes**: Harvey usa proprietárias (`HarveySerifFont` custom + Diatype/Dinamo, pagas, não embarcáveis). `--font-serif` **em teste pelo dono** — atualmente **Times New Roman** (fonte de sistema). Histórico testado contra a HarveySerif real: Source Serif 4, Fraunces (mais fiel), Playfair (contraste alto), Newsreader (arredondada ❌), Libre Caslon/Prata (leves). `--font-sans` = **Inter** (≈ Diatype). `--green`/`--red` só como feedback semântico.
